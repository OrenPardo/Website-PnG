...
export default function LangToggle({ variant = 'default' }) {
  ...
  const isSubpageWhite = variant === 'subpage';

  return (
    <button
      type="button"
      onClick={toggle}
      className={`lang-btn flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded transition cursor-pointer ${
        isSubpageWhite
